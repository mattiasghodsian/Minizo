...
export const getMeta = (req, res) => {
  ...
  execa('youtube-dl', ['--dump-json', url])
    .then(({ stdout }) => {
      const data = JSON.parse(stdout);
      const thumbnail = data.thumbnails[Object.keys(data.thumbnails)[Object.keys(data.thumbnails).length - 1]];
